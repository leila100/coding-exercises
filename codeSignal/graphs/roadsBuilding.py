...
Over the years most of the two-way roads built by Byteasar's predecessors were forgotten and no longer traversable. 
...
Desperate, he turned to his magic crystal ball for help. 
The crystal showed him a programmer from the distant future: you! 
Now you're the only one who can save the kingdom. Given the existing roads and the number of cities in the kingdom, 
...
the output should be roadsBuilding(cities, roads) = [[0, 3], [1, 3], [2, 3]].
...
Array of roads in the kingdom. 
Each road is given as a pair [cityi, cityj], where 0 ≤ cityi, cityj < cities and cityi ≠ cityj. 
It is guaranteed that no road is given twice.
...
A unique array of roads that should be built sorted as described above. 
There's no need to build looping roads, i.e. roads that lead back from a city to itself.
'''

from operator import itemgetter, attrgetter
    
def roadsBuilding(cities, roads):
    # build set of connections for each city
    connections = {}
    ...
    for i in range(cities):
        for city in connections:
            if i != city and i not in connections[city]:
                if i < city:
                    connect = (i, city)
                else:
                    connect = (city, i)
    ...
    response.sort(key=itemgetter(0,1))
    return response
