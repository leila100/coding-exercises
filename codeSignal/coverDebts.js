...
if at the end of the month, you do not fully pay your ith debt, its amount increases by 
(interests[i] / 100) * debts[i] dollars. In other words, if at the end of the month for some i where debts[i] > 0, 
...
After that the debts array will look like [0, 2, 2]. After recalculations it will look like 
[0, 2 + 2 * (100 / 100), 2 + (2 * 150 / 100)] = [0, 4, 5].
During the 2nd month, spend $5 to cover debts[2]. After that the debts array will look like [0, 4, 0]. 
After recalculations it will look like [0, 4 + 4 * (100 / 100), 0] = [0, 8, 0].
During the 3rd month, spend $5 to partially cover debts[1]. After that the debts array will look like [0, 3, 0]. 
...
After that the debts array will look like [0, 1, 0]. After recalculations the debts array will look like 
[0, 1 + 1 * (100 / 100), 0] = [0, 2, 0].
...
the output should be coverDebts(s, debts, interests) = 10.8125.
...
After that the debts array will look like [0, 0, 5]. After recalculations it will look like 
[0, 0, 5 + 5 * (25 / 100)] = [0, 0, 6.25].
During the 2nd month, spend $4 to partially cover debts[2]. 
After that the debts array will look like [0, 0, 2.25]. After recalculations it will look like 
[0, 0, 2.25 + 2.25 * (25 / 100)] = [0, 0, 2.8125].
...
The minimal amount of money can be spent to cover all the debts. 
Your answer will be considered correct if absolute or relative error won't exceed 10-5.
*/
function coverDebts(s, debts, interests) {
  // build an array with couples where couple[0]=interest and couple[1] =corresponding dept
  const couples = [];
  for (let i = 0; i < interests.length; i++) {
    couples.push([interests[i], debts[i]]);
  }
  // sort interests array because we'll select debts based on it
  couples.sort((a, b) => b[0] - a[0]);
  const payEachMonth = (s * 10) / 100;
  let totalMoney = 0;
  let currentMoney = payEachMonth;
  for (let i = 0; i < couples.length; i++) {
    while (couples[i][1] > 0) {
      if (couples[i][1] >= currentMoney) {
        couples[i][1] -= currentMoney;
        for (let j = i; j < couples.length; j++) {
          couples[j][1] += couples[j][1] * (couples[j][0] / 100);
        }
        totalMoney += currentMoney;
        currentMoney = payEachMonth;
      } else {
        currentMoney -= couples[i][1];
        totalMoney += couples[i][1];
        couples[i][1] = 0;
      }
    }
  }
...
const interests = [200, 100, 150];
const debts = [2, 2, 5];
const s = 50;

console.log(coverDebts(s, debts, interests));
