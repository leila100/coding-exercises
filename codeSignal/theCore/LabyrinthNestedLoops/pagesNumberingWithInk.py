...
Now you want to write a function to determine what the last page of the book is that you can 
number given the current page and numberOfDigits left. 
A page is considered numbered if it has the full number printed on it 
(e.g. if we are working with page 102 but have ink only for two digits then this 
...
def pagesNumberingWithInk(current, numberOfDigits):
  currentNum = numberOfDigits - len(str(current))
  while currentNum >= 0:
    current += 1
    currentNum -= len(str(current))
    if currentNum < 0:
        return current - 1
    if currentNum == 0:
        return current
