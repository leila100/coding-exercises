...
Unfortunately, Internet traffic is very expensive in the North Pole, so instead of 
sending an actual image he got creative and drew the tree using nothing but asterisks 
...
The Christmas tree according to the specs and inputs. 
Output elements should not contain trailing whitespaces, 
and at least one of them should start with the '*' symbol.
'''

def christmasTree(levelNum, levelHeight):
    # use the middle asterisk as the basis for all the lines in the tree
    middle = "*"
    tree = [middle, middle, "***"]
    num = 1
    for numLevel in range(levelNum):
        for level in range(1, levelHeight+1):
            newLine = middle + ("**" * (level+num))
            tree.append(newLine)
        num += 1
    # Add foot
    if levelHeight % 2 == 0:
    ...
    for i, level in enumerate(tree):
        spacesLength = (len(tree[-levelNum-1]) - len(level)) // 2
        spaces = "".join([" " for scape in range(spacesLength)])
        tree[i] = spaces + level
            
    return tree

# tree = christmasTree(1, 3) 
tree = christmasTree(2, 4) 
for line in tree:
    print(line)