...
function mostViewedWriters(topicIds, answerIds, views) {
  ...
  for (let topic in topics) {
    const topicViews = {};
    let answers = new Set();
    const top = Array.from(topics[topic]);
    for (let i = 0; i < top.length; i++) {
      for (let j = 0; j < answerIds[top[i]].length; j++) answers.add(answerIds[top[i]][j]);
    }
    const viewsArr = views.filter((view) => answers.has(view[0]));
    for (let i = 0; i < viewsArr.length; i++) {
      const viewsNum = viewsArr[i][2];
      const user = viewsArr[i][1];
      if (user in topicViews) topicViews[user] += viewsNum;
      else topicViews[user] = viewsNum;
    }
    const t = [];
    for (user in topicViews) t.push([parseInt(user), topicViews[user]]);
    t.sort((a, b) => b[1] - a[1]);
    users.push(t);
  }
  return users;
...
console.log(mostViewedWriters(topicIds, answerIds, views));
