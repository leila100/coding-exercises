...
function proCategorization(pros, preferences) {
  ...
  for (let i = 0; i < preferences.length; i++) {
    for (let category of preferences[i]) {
      if (category in prosCategories) {
        prosCategories[category].push(pros[i]);
      } else {
        prosCategories[category] = [pros[i]];
      }
    }
  }
  //   console.log(prosCategories)
  const solution = [];
  const cats = Object.keys(prosCategories);
  cats.sort();

  for (let category of cats) {
    const sortedPros = prosCategories[category].sort();
    const prosCat = [[category], [...sortedPros]];
    solution.push(prosCat);
  }
