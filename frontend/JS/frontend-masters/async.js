/*
Asynchronicity
Note: We recommend that after you complete a challenge, you re-comment out the test case for that challenge so the console 
output is not confusing when working on subsequent challenges.
...
function everyXsecsForYsecs(func, interval, duration) {
  ...
  function printMessage() {
    func();
    counter += interval;
    if (counter >= duration) clearInterval(printEverySecond);
  }
  var printEverySecond = setInterval(printMessage, interval);
}
...
function delayCounter(target, wait) {
  ...
  function printMessage() {
    counter++;
    console.log(counter);
    if (counter >= target) clearInterval(intervalVar);
  }
  return () => (intervalVar = setInterval(printMessage, wait));
}
...
function promised(val) {
  return new Promise((resolve, reject) => setTimeout(() => resolve(val), 2000));
}
...
class SecondClock {
  ...
  printSeconds = () => {
    this.secondsVal = this.secondsVal % 60;
    this.secondsVal++;
    this.cb(this.secondsVal);
  };
  start() {
    this.stopInterval = setInterval(this.printSeconds, 1000);
  }
  reset() {
