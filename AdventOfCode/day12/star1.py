...
Simulate the motion of the moons in time steps. Within each time step, first update the velocity of every 
moon by applying gravity. Then, once all moons' velocities have been updated, update the position of every 
moon by applying velocity. Time progresses by one step once all of the positions are updated.
...
def init_moons(moons):
    # f = open("input1.txt", "r")
    # f = open("input2.txt", "r")
    f = open("input.txt", "r")
    moon1 = f.readline().strip()[1:-1].split(',')
    moon1 = map(lambda x: int(x.split('=')[1]), moon1)
    moon2 = f.readline().strip()[1:-1].split(',')
    moon2 = map(lambda x: int(x.split('=')[1]), moon2)
    moon3 = f.readline().strip()[1:-1].split(',')
    moon3 = map(lambda x: int(x.split('=')[1]), moon3)
    moon4 = f.readline().strip()[1:-1].split(',')
    moon4 = map(lambda x: int(x.split('=')[1]), moon4)

    moons['moon1']['position'] = list(moon1)
    moons['moon2']['position'] = list(moon2)
    moons['moon3']['position'] = list(moon3)
    moons['moon4']['position'] = list(moon4)
...
def apply_velocity(moons):
    for moon in moons:
        new_position = []
        velocities = moons[moon]['vel']
        positions = moons[moon]['position']
        for i in range(3):
            new_position.append(velocities[i] + positions[i])
        moons[moon]['position'] = new_position


def apply_step(moons):
    moons_names = list(moons.keys())
    for i in range(4):
        for j in range(i+1, 4):
            moon1 = moons[moons_names[i]]
            moon2 = moons[moons_names[j]]
            apply_gravity(moon1, moon2)
    apply_velocity(moons)


def add_pot_energy(moons):
    for moon in moons:
        x = moons[moon]['position'][0]
        y = moons[moon]['position'][1]
        z = moons[moon]['position'][2]
        pot = abs(x) + abs(y) + abs(z)
        moons[moon]['pot'] = pot


def add_kin_energy(moons):
    for moon in moons:
        x = moons[moon]['vel'][0]
        y = moons[moon]['vel'][1]
        z = moons[moon]['vel'][2]
        kin = abs(x) + abs(y) + abs(z)
        moons[moon]['kin'] = kin


def add_total_energy(moons):
    for moon in moons:
        pot = moons[moon]['pot']
        kin = moons[moon]['kin']
        total = pot * kin
        moons[moon]['total'] = total
...
def total_energy():
    ...
    for step in range(1000):
        apply_step(moons)
    add_pot_energy(moons)
    add_kin_energy(moons)
    add_total_energy(moons)
    print(f'result: {moons}')
    total = 0
    for moon in moons:
        total += moons[moon]['total']
    return total
