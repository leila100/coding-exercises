...
that should run on your existing Intcode computer. Each amplifier will need to run a copy of the program.
...
def decode(codes, sequenceVal, inputVal=0):
    counter = 0
    first = True
    returnVal = None
...
def getThrustSignal(sequence):
    inputVal = None
    f = open("input.txt", "r")
    input1 = f.readline()
    codes = input1.split(",")
    for i in range(5):
        inputVal = decode(codes, sequence[i], inputVal)
    return inputVal

def findHighestSignal():
    maxSignal = None
    sequencePermutations = [p for p in permutations(range(5))]
    for sequence in sequencePermutations:
        signal = getThrustSignal(sequence)
        if maxSignal is None or int(signal) > int(maxSignal):
            maxSignal = signal
